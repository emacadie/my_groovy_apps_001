...
class ShiroDbRealm {
    ...
    def isPermitted(principal, requiredPermission) {
        ...
        // First find all the permissions that the user has that match the required permission's type and project code.
        def user = ShiroUser.findByUsername(principal)
        def permissions = user.permissions

        // Try each of the permissions found and see whether any of them confer the required permission.
        ...
        // There may be some duplicate entries in the results, but at this stage it is not worth trying to remove them. Now,
        // create a real permission from each result and check it against the required one.
