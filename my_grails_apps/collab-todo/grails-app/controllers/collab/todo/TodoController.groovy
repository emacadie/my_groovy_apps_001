...
class TodoController {
    ...
    def beforeAudit = {
        log.trace("${session?.person?.personName} Start action ${controllerName}Controller.${actionName}() : parameters $params")
    }
    
    def afterAudit = { model ->
        log.trace("${session?.person?.personName} End action ${controllerName}Controller.${actionName}() : returns $model")
    }
        ...
        if (version != null) {
            if (todoInstance.version > version) {
                todoInstance.errors.rejectValue("version", "default.optimistic.locking.failure",
                          ...
                          "Another person has updated this Todo while you were editing")
