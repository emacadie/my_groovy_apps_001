...
class FirstMutableExSampleSpec extends Specification { 
    ...
    def "test the no arg constructor"() {
        ...
        // def junk = new FirstMutableSample()
        println "About to make throwaway"
        def throwaway = new FirstMutableExSample( firstString: "Not Junk", secondString: "Goodbye Junk", firstInt: 21 )
        throwaway.firstString = "Throwaway"
        println "Just made throwaway, about to make bTest1"
        def bTest1 = new FirstMutableExSample( firstString: "Hello1", secondString: "Goodbye", firstInt: 21, secondInt: 2000 )
    ...
    def "test bTest2"() {
        println "--- Starting test ${name.methodName}"
        def bTest2 = new FirstMutableExSample( firstString: "Hello2", secondString: "Goodbye, this is more than 20 characters", firstInt: 22, secondInt: 20 )
    ...
    def "test the no arg constructor again"() {
        println "--- Starting test ${name.methodName}"
        /*
        def constructors = FirstMutableSample.class.getConstructors()
        constructors.each { 
            println "Constructor: ${it.toString()}"
        }
        def methods = FirstMutableSample.metaClass.getMetaMethods()
        methods.each {
            println "MetaMethod: ${it.toString()}"
        }
        */
        
        def bTest1 = new FirstMutableExSample( firstString: "Hello3", secondString: "Goodbye", firstInt: 21, secondInt: 401 )
        println "In test ${name.methodName}, bTest1: ${bTest1.toString()}"
        expect:
